...
export async function login(loginName: string, password: string): Promise<AuthResponse> {
  ...
  const profile = await getProfile(data.user.id);
  if (profile.is_banned) { await supabase.auth.signOut(); throw new Error(`Your account has been suspended: ${profile.ban_reason ?? "Violation of terms."}`); }
  setToken(data.session.access_token);
  return { token: data.session.access_token, user: toPublicUser(profile, data.user.email ?? email) };
}

export async function currentSession() { const { data: { user }, error } = await supabase.auth.getUser(); if (error || !user) throw new Error("Not signed in."); const profile = await getProfile(user.id); return { user: toPublicUser(profile, user.email ?? "") }; }
