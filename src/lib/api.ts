...
export async function currentSession() {
  return request<{ user: PublicUser }>("/api/session");
}
...
export async function deleteAccount() {
  const result = await request<{ ok: true }>("/api/account/delete", { method: "POST" });
  setToken(null);
  return result;
}
...
export async function blockPlayer(targetUsername: string) {
  return request<{ user: PublicUser }>("/api/block", {
    method: "POST",
...
export async function getHistory() {
  return request<{ games: GameRecord[] }>("/api/history");
}

export async function updateSettings(settings: Partial<UserSettings>) {
  return request<{ user: PublicUser }>("/api/settings", {
    method: "PATCH",
...
export async function saveGame(game: GameRecord) {
  return request<{ game: GameRecord }>("/api/games", {
    method: "POST",
...
// --- Admin API Methods (Owner Only) ---
export async function verifyOwnerPassword(password: string) {
  return request<{ ok: true }>("/api/admin/verify-owner", {
    method: "POST",
...
export async function banUser(userId: string, isBanned: boolean, banReason?: string) {
  return request<{ user: PublicUser }>("/api/admin/users/ban", {
    method: "POST",
    body: JSON.stringify({ userId, isBanned, banReason })
  });
}

export async function setUserRatingAdmin(userId: string, rating: number) {
  return request<{ user: PublicUser }>("/api/admin/users/rating", {
    method: "POST",
...
export async function getAdminReports() {
  return request<{ reports: ReportItem[] }>("/api/admin/reports");
}

export async function updateReportStatus(reportId: string, status: "resolved" | "dismissed") {
  return request<{ ok: true }>("/api/admin/reports/status", {
    method: "POST",
...
export async function getAdminAnalytics() {
  return request<{ analytics: SystemAnalytics }>("/api/admin/analytics");
}

export async function getAuditLogs() {
  return request<{ auditLogs: AuditLog[] }>("/api/admin/audit-logs");
}

export async function createAnnouncement(title: string, content: string) {
  return request<{ announcement: Announcement }>("/api/admin/announcements", {
    method: "POST",
